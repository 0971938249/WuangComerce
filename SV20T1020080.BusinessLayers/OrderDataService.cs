...
namespace SV20T1020080.BusinessLayers
{
    public static class OrderDataService
    {
        ...
        /// </summary> 
        public static List<Order> ListOrders(out int rowCount, int page = 1, int pageSize = 0, int status = 0, DateTime? fromTime = null, DateTime? toTime = null,
        string searchValue = "")
        {
        ...
        public static bool SaveAddress(int OrderID = 0, string deliveProvince = "", string deliveryAddress = "")
        {
            Order? data = orderDB.Get(OrderID);
            if (data == null)
                return false;
            if (data.Status == Constants.ORDER_INIT || data.Status == Constants.ORDER_ACCEPTED)
            {
                return orderDB.SaveAddress(OrderID, deliveProvince, deliveryAddress);
            }
            return false;
        }
        /// <summary> 
